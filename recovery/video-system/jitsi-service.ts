...
export class JitsiService {
  ...
  async initialize(containerId: string): Promise<void> {
    try {
      console.log('📡 Jitsi Public: Starting initialization...');
      
      // Load Jitsi Meet API if not already loaded
      console.log('📡 Jitsi Public: Loading Jitsi API...');
      await this.loadJitsiAPI();
      console.log('📡 Jitsi Public: Jitsi API loaded successfully');

      this.container = document.getElementById(containerId);
      if (!this.container) {
        throw new Error(`Container with id ${containerId} not found`);
      }
      console.log('📡 Jitsi Public: Container found, creating meeting...');
      ...
      const options = {
        ...
        configOverwrite: {
          ...
          // Mobile network optimizations for Africa
      ...
      // Create Jitsi Meet API instance - use public Jitsi for reliability
      const jitsiDomain = process.env.NEXT_PUBLIC_JITSI_PUBLIC_DOMAIN || 'meet.jit.si';
      console.log('📡 Jitsi: Using public domain:', jitsiDomain);
  ...
  private async loadJitsiAPI(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // Create script element - use public servers for reliability
    ...
    this.api.addEventListener('endpointTextMessageReceived', (event: any) => {
      try {
        ...
        if (data.type === 'device-info' && data.deviceInfo) {
          console.log('📱 Jitsi: Received device info from remote user:', data.deviceInfo);
          this.callbacks.onRemoteDeviceInfo?.(data.deviceInfo);
        }
      } catch (error) {
        console.warn('📱 Jitsi: Failed to parse endpoint text message:', error);
      }
    });
  ...
  async toggleAudio(): Promise<boolean> {
    console.log('🎤 JitsiService: toggleAudio called');
    if (this.api) {
      this.api.executeCommand('toggleAudio');
      console.log('🎤 Jitsi toggleAudio command executed');
      
      // Toggle our tracked state
      this.isAudioMuted = !this.isAudioMuted;
      
      console.log(`🎤 Jitsi audio state toggled to muted: ${this.isAudioMuted}`);
      return this.isAudioMuted;
    } else {
      console.warn('🎤 Jitsi API not available for toggleAudio');
      return this.isAudioMuted;
    }
  }

  async toggleVideo(): Promise<boolean> {
    console.log('📹 JitsiService: toggleVideo called');
    if (this.api) {
      this.api.executeCommand('toggleVideo');
      console.log('📹 Jitsi toggleVideo command executed');
      
      // Toggle our tracked state
      this.isVideoOff = !this.isVideoOff;
      
      console.log(`📹 Jitsi video state toggled to off: ${this.isVideoOff}`);
      return this.isVideoOff;
    } else {
      console.warn('📹 Jitsi API not available for toggleVideo');
      return this.isVideoOff;
    }
  }
  ...
  getCurrentStates(): { isAudioMuted: boolean; isVideoOff: boolean } {
    console.log(`🔧 JitsiService: getCurrentStates called - audio muted: ${this.isAudioMuted}, video off: ${this.isVideoOff}`);
    return {
  ...
  setInitialMuteStates(audioMuted: boolean, videoOff: boolean): void {
    console.log(`🔧 JitsiService: Setting initial states - audio muted: ${audioMuted}, video off: ${videoOff}`);
  ...
  sendDeviceInfo(deviceInfo: any): void {
    if (this.api) {
      try {
        ...
        console.error('Failed to send device info via Jitsi:', error);
      }
    }
  }
}